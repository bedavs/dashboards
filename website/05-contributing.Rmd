...
3. Fork the following repos, and clone them to your computer inside `~/git/dashboards`:
    - https://github.com/raubreywhite/dashboards_control/ (private)
4. For each repo, add the raubreywhite repository as your upstream:

```
git remote add upstream https://github.com/raubreywhite/dashboards_control.git
```

5. Fork the following repos, and then clone the forks to your computer inside `~/git/dashboards`:
...
6. For each repo, add the folkehelseinstitutet repository as your upstream:

```
git remote add upstream https://github.com/folkehelseinstituttet/ORIGINAL_REPOSITORY.git
```

7. In your `~/.profile` add the following three lines:
...
8. Build your Dockerfiles:

```
docker_build.sh
```

9. A number of folders will have been automatically created (see below). Please put in your development datafiles into the appropriate `data_raw/` folder:
...
11. Open a browser and go to http://localhost:8788/
12. Login using `username=rstudio` and `password=rstudio1`
13. Using the project menu, open the project corresponding to the automated analysis you are interested in. This should be located at `/dashboards/`.
