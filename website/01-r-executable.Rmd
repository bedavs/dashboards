...
The R executable (commonly called `RunProcess.R`) is responsible for these tasks.
...
1. Data files need to be downloaded from external SFTP servers ([normomo](https://folkehelseinstituttet.github.io/dashboards_normomo/), [sykdomspuls_log](https://folkehelseinstituttet.github.io/dashboards_sykdomspuls_log/)).
...
1. The bounded context of `RunProcess.R` happens
2. Run `RunProcess.R`

Point #1 is done through the use of `flock`.

(If neccessary) point #2 uses `sshpass`, `sftp`, and `ncftpput` to download/upload files from SFTP servers.
